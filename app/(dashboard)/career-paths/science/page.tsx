...
function CareerIllustration({ title, gradient }: { title: string; gradient: string }) {
    ...
    return (
        <div className="relative w-full h-28 rounded-2xl overflow-hidden flex items-center justify-center mb-4"
            ...
            <div className="absolute -bottom-4 -left-4 w-16 h-16 rounded- opacity-15" style={{ background: 'rgba(255,255,255,0.5)' }} />
            {/* Icon */}
...
function CareerCard({ career, gradient, index }: { career: CareerPath; gradient: string; index: number }) {
    ...
    return (
        <div
            className="h-full rounded-3xl overflow-hidden flex flex-col hover:scale-[1.02] transition-all duration-300 group cursor-pointer"
            style={{
        ...
        >
            ...
            <div className="relative h-36 flex items-center justify-center overflow-hidden" style={{ background: gradient }}>
                {/* Decorative blobs */}
                <div className="absolute -top-8 -right-8 w-24 h-24 rounded-full opacity-20 sm:w-32 sm:h-32" style={{ background: 'rgba(255,255,255,0.5)' }} />
                <div className="absolute -bottom-6 -left-6 w-16 h-16 rounded-full opacity-15 sm:w-20 sm:h-20" style={{ background: 'rgba(255,255,255,0.4)' }} />
                {/* Title strip */}
                <div className="absolute inset-x-0 bottom-0 px-4 py-3 flex items-center gap-2"
                    ...
                    <h3 className="text-white font-bold text-sm sm:text-base leading-tight flex-1">{career.title}</h3>
                    <button
                ...
                {/* Big icon in center */}
                <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl flex items-center justify-center relative z-10"
            ...
            <div className="flex-1 p-4 sm:p-5 flex flex-col">
                <p className="text-sm sm:text-base text-slate-600 mb-3 leading-relaxed line-clamp-2">{career.description}</p>

                {/* Stats row */}
                <div className="flex flex-col gap-3 mb-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-center gap-2">
                        <DollarSign className="w-4 h-4 text-green-600" />
                        <span className="text-sm font-semibold text-slate-700">{career.salary_range || '₹6-12 LPA'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <Target className="w-4 h-4" style={{ color: getGrowthColor(career.growth_rate) }} />
                        <span className="text-sm font-medium" style={{ color: getGrowthColor(career.growth_rate) }}>
                ...
                {/* Difficulty badge */}
                <div className="flex flex-col gap-3 mb-3 sm:flex-row sm:items-center sm:justify-between">
                    <span className="px-2 py-1 rounded-full text-xs sm:text-sm font-bold text-white"
                        style={{ background: getDifficultyColor(career.difficulty) }}>
                        {career.difficulty || 'Medium'}
                    </span>
                    <span className="text-sm text-slate-500">{career.degree_required}</span>
                </div>
                ...
                <div className="mb-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {career.preparation_for.slice(0, 2).map((item, i) => (
                            <span key={i} className="px-3 py-1 rounded-full text-xs sm:text-sm font-medium text-slate-600"
                                style={{ background: 'rgba(103,126,234,0.08)', border: '1px solid rgba(103,126,234,0.15)' }}>
                                {item}
                            </span>
                        ))}
                        {career.preparation_for.length > 2 && (
                            <span className="px-3 py-1 rounded-full text-xs sm:text-sm font-medium text-slate-400"
                    ...
                    <div className="mb-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {career.job_opportunities.slice(0, 3).map((job, i) => (
                                <span key={i} className="px-3 py-1 rounded-full text-xs sm:text-sm font-medium text-slate-500"
                ...
                <button
                    ...
                    className="mt-auto w-full py-3 rounded-2xl text-white text-sm font-bold flex items-center justify-center gap-2 transition-all hover:opacity-90 active:scale-95 cursor-pointer"
